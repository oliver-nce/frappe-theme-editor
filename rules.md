# Frappe Manager — Agent Rules

## Project Context
Read `docs/CURRENT-STATE-HANDOFF.md` for full architecture, file map, database schema, API endpoints, and deployment details.

## Key Architecture
- Node.js backend (api-server.js port 3000, terminal-helper.js port 8765)
- Vanilla JS frontend (no build step)
- Local MariaDB (frappe_manager) + remote Frappe site DBs via frappe_readonly
- Frappe site data queried directly (frappe-db-query.js), not mirrored locally
- SSH discovery for bench/site structure, direct DB queries for Frappe data
- PM2 + Apache reverse proxy on AWS Lightsail
